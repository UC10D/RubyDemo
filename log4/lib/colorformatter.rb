...
# Change from log4r/formatter/patternformatter.rb
module Log4r
  class ColorFormatter < BasicFormatter
    ...
    DirectiveTable = {
      ...
      'L' => %q(case LNAMES[event.level]
                when "ERROR"
                  "\e[31m#{LNAMES[event.level]}\e[0m"
                when "WARNING"
                  "\e[33m#{LNAMES[event.level]}\e[0m"
                when "INFO"
                  "\e[32m#{LNAMES[event.level]}\e[0m"
                else
                  LNAMES[event.level][0..4]
                end),
      'x' => 'Log4r::NDC.get()',
    ...
    def initialize(hash = {})
      super(hash)
      @pattern = (hash['pattern'] || hash[:pattern] || nil)
      @date_pattern = (hash['date_pattern'] || hash[:date_pattern] || nil)
      @date_method = (hash['date_method'] || hash[:date_method] || nil)
    ...
    def PatternFormatter.create_format_methods(pf) #:nodoc:
      ...
      loop do # work on each match in turn
